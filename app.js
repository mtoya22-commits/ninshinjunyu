const PIN_HASH = "3472adbbcb9677d1b45365d37d96d1c33217d745567577fd9bd5c2766a258320";
    ...
    function getBulkTermSegments(text) {
      ...
      while ((match = re.exec(text)) !== null) {
        ...
        const committed = delimiter.length > 0 || end < text.length;
    ...
    function analyzeBulkInput() {
      const segments = getBulkTermSegments(bulkInput.value);
      bulkChosen = new Map();
    ...
    function updateBulkAddButton() {
      ...
      const totalTerms = getBulkTermSegments(bulkInput.value).length;
    ...
    function addBulkSelected() {
      ...
      // 結果表示：モーダルを確実に閉じ、キーボードを閉じてから結果エリアへ移動
      closeBulkModal({ skipFocusBack: true });
...
// Network-first service worker registration.
    ...
    const APP_VERSION = "20260505-0044";
