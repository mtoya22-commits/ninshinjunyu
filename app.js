const PIN_HASH = "3472adbbcb9677d1b45365d37d96d1c33217d745567577fd9bd5c2766a258320";
    ...
    function statusType(summary, insert, axis) {
      ...
      if (s.includes("詳細参照")) return "caution";
    ...
    function renderSuggestions(items) {
      ...
      if (!items.length) {
        suggestionsEl.innerHTML = '<div class="hint">該当候補がありません。例：商品名・成分名・ひらがなで検索してください。</div>';
        return;
      }
    ...
    function renderResults() {
      ...
      const row = (label, sub, contentFn, rowClass = "") => `
        <tr class="${rowClass}">
          <th>
          ...
          ${ranked.map((drug) => {
            ...
            const cellClass = rowClass === "insert-row" ? "insert-cell" : badgeClass(type);
            return `<td class="${cellClass}">${contentFn(drug, type)}</td>`;
          }).join("")}
        </tr>
      `;
      ...
      resultEl.innerHTML = `
        ...
        <div class="rx-table-wrap v29-table-wrap ${ranked.length <= 3 ? "fit-three" : "scroll-table"}" aria-label="${axisLabel}の薬剤比較表">
          <table class="rx-table v29-table" data-drug-count="${ranked.length}">
            <thead>
              <tr>
                <th>評価項目</th>
                ${ranked.map((drug, index) => `
                  <th>
                    <div class="rx-table-headline"><span class="rx-col-num">${index + 1}</span><button type="button" class="rx-table-title" data-drug-id="${escapeHtml(drug.id)}">${escapeHtml(drug.brands[0])}</button></div>
      ...
      bindInsertHelpButtons(resultEl);
    }
    ...
    function getDisplayInsert(value, axis = currentMode) {
      const v = String(value ?? "").trim();

      // データ上の未記載表記は統一
      if (v === "—" || v === "-" || v === "―" || v === "" || v === "記載なし") return "記載なし";
      ...
      if (axis === "lactation") {
        if (v === "添文①") return "授乳回避（添文①）";
        if (v === "添文②") return "授乳しないことが望ましい（添文②）";
        if (v === "添文③") return "継続または中止を検討（添文③）";
        if (v === "添文④") return "投与回避・やむを得ず投与なら授乳中止（添文④）";
      }

      return v;
    }

    function getInsertHelpInfo(rawValue, axis = currentMode) {
      const label = getDisplayInsert(rawValue, axis);

      const pregnancy = {
        "使用可": "妊娠中でも比較的使用しやすいと判断される薬剤です。ただし適応、用量、妊娠週数、患者背景を確認してください。",
        "詳細参照": "一律の可否ではなく、薬剤ごとの詳細本文を確認して判断する分類です。",
        "有益性（添文②）": "添付文書上、「治療上の有益性が危険性を上回ると判断される場合にのみ投与する」趣旨の記載がある薬剤です。",
        "有益性（添文③）": "添付文書上、「治療上の有益性を考慮して投与する」趣旨の記載がある薬剤です。",
        "記載なし": "添付文書上、妊娠に関する明確な分類記載が確認できない薬剤です。",
        "使用不可": "妊娠中の使用を避ける、または禁忌・原則使用不可と判断される薬剤です。"
      };

      const lactation = {
        "使用可": "授乳中でも比較的使用しやすいと判断される薬剤です。ただし授乳児の月齢、早産児・低出生体重児、基礎疾患、用量を確認してください。",
        "詳細参照": "一律の可否ではなく、薬剤ごとの詳細本文を確認して判断する分類です。",
        "授乳回避（添文①）": "添付文書上、授乳を避ける趣旨の記載がある薬剤です。",
        "授乳しないことが望ましい（添文②）": "添付文書上、投与中は授乳しないことが望ましい等の記載がある薬剤です。",
        "継続または中止を検討（添文③）": "添付文書上、授乳の継続または中止を検討する趣旨の記載がある薬剤です。",
        "投与回避・やむを得ず投与なら授乳中止（添文④）": "添付文書上、投与を避ける、またはやむを得ず投与する場合は授乳中止を検討する趣旨の記載がある薬剤です。",
        "記載なし": "添付文書上、授乳に関する明確な分類記載が確認できない薬剤です。",
        "使用不可": "授乳中の使用を避ける、または原則使用不可と判断される薬剤です。"
      };

      const map = axis === "lactation" ? lactation : pregnancy;
      return {
        label,
        axisLabel: axis === "lactation" ? "授乳" : "妊娠",
        body: map[label] || "この分類の詳細は薬剤ごとの本文、添付文書、患者背景を確認してください。"
      };
    }

    function renderInsertHelpButton(rawValue, axis = currentMode, className = "rx-mini-chip") {
      const info = getInsertHelpInfo(rawValue, axis);
      return `<button type="button" class="${className} insert-help-chip" data-insert-help="1" data-insert-axis="${escapeHtml(axis)}" data-insert-value="${escapeHtml(String(rawValue ?? ""))}">${escapeHtml(info.label)} <span class="help-mark">?</span></button>`;
    }

    function renderInsertHelpButtonCompact(rawValue, axis = currentMode) {
      const compactLabel = getInsertTableShortLabel(rawValue, axis);
      const stateClass = getInsertCompactState(rawValue, axis);
      return `
        <button type="button"
          class="insert-compact-chip ${stateClass}"
          data-insert-help="1"
          data-insert-axis="${escapeHtml(axis)}"
          data-insert-value="${escapeHtml(String(rawValue ?? ""))}"
          aria-label="添付文書分類の詳細を表示: ${escapeHtml(compactLabel)}">
    ...
    function getInsertTableShortLabel(rawValue, axis = currentMode) {
      const value = String(rawValue ?? "").trim();
      if (value === "—" || value === "-" || value === "―" || value === "" || value === "記載なし") return "記載なし";
      if (axis === "pregnancy") {
        if (value === "添文②") return "添文②";
        if (value === "添文③") return "添文③";
      }
      if (axis === "lactation") {
        if (value === "添文①") return "添文①";
      ...
      if (value === "有益性（添文②）") return "添文②";
      if (value === "有益性（添文③）") return "添文③";
      if (value === "授乳回避（添文①）") return "添文①";
      if (value === "授乳しないことが望ましい（添文②）") return "添文②";
      if (value === "継続または中止を検討（添文③）") return "添文③";
      if (value === "投与回避・やむを得ず投与なら授乳中止（添文④）") return "添文④";
      return value;
    }

    function getInsertCompactState(rawValue, axis = currentMode) {
      const shortLabel = getInsertTableShortLabel(rawValue, axis);
      const longLabel = getDisplayInsert(rawValue, axis);
      if (longLabel.includes("使用不可") || longLabel.includes("禁忌") || shortLabel === "添文④") return "danger";
      if (longLabel.includes("使用可")) return "ok";
      if (longLabel.includes("記載なし") || longLabel.includes("情報なし") || longLabel.includes("情報不足")) return "unknown";
      return "caution";
    }


    function openInsertHelp(rawValue, axis = currentMode) {
      const info = getInsertHelpInfo(rawValue, axis);
      metricHelpTitle.textContent = `${info.label}：${info.axisLabel}分類`;
      metricHelpBody.innerHTML = `
        <div class="metric-help-content insert-help-content">
          <p>${escapeHtml(info.body)}</p>
          <p class="help-note">この分類は添付文書記載の要旨を示すものであり、実際の使用可否は薬剤ごとの本文、患者背景、妊娠週数、授乳児の月齢、用量、投与期間を踏まえて判断してください。</p>
        </div>
      `;
    ...
    function bindInsertHelpButtons(root = document) {
      root.querySelectorAll("[data-insert-help]").forEach(btn => {
        if (btn.dataset.insertHelpBound === "1") return;
        btn.dataset.insertHelpBound = "1";
        btn.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
          openInsertHelp(btn.dataset.insertValue || "", btn.dataset.insertAxis || currentMode);
        });
      });
    }
        ...
        <article class="rx-item">
          <button class="rx-title" type="button" data-drug-id="${escapeHtml(drug.id)}">
            <div>
              ...
              <div class="mini-eval">
                ...
                <span>添付文書：${renderInsertHelpButton(getAxisInsert(drug), currentMode, "inline-insert-help")}</span>
              </div>
            </div>
          ...
          <div class="rx-body">
            <div class="status-grid">
              ${statusBox(axisLabel, getAxisSummary(drug), getAxisInsert(drug), type, currentMode)}
            </div>
    ...
    function statusBox(label, summary, insert, type, axis = currentMode) {
      const axisName = label.includes("妊娠") ? "妊娠" : label.includes("授乳") ? "授乳" : label;
      return `
        <div class="status-box axis-status-box axis-status-box-${badgeClass(type)}">
          <div class="axis-status-head">
            <div>
              <div class="axis-status-label">${escapeHtml(axisName)}の総合評価</div>
              <div class="axis-status-sub">総合評価を最優先に確認</div>
            </div>
            <span class="axis-status-rank ${badgeClass(type)}">${statusText(type)}</span>
          </div>
          <div class="axis-main-result">
            <span class="axis-main-caption">総合評価</span>
            <span class="badge ${badgeClass(type)} axis-main-badge">${escapeHtml(summary)}</span>
          </div>
          <div class="axis-insert-row">
            <span class="axis-insert-label">添付文書</span>
            <span class="axis-insert-value">${renderInsertHelpButton(insert, axis, "inline-insert-help")}</span>
          </div>
        </div>
      `;
    }
    ...
    function bulkChoiceKey(idx, term) {
      return `${idx}:${normalize(term)}`;
    }

    let bulkManualChoices = new Map();

    function openBulkModal() {
      bulkModal.classList.remove("force-closed");
      bulkModal.style.removeProperty("display");
      bulkModal.style.removeProperty("visibility");
      bulkModal.style.removeProperty("opacity");
      bulkModal.style.removeProperty("pointer-events");
    ...
    function closeBulkModal(options = {}) {
      bulkModal.classList.add("force-closed");
      bulkModal.classList.remove("open");
      bulkModal.setAttribute("aria-hidden", "true");
      bulkModal.style.setProperty("display", "none", "important");
      bulkModal.style.setProperty("visibility", "hidden", "important");
      bulkModal.style.setProperty("opacity", "0", "important");
      bulkModal.style.setProperty("pointer-events", "none", "important");
    ...
    function analyzeBulkInput() {
      ...
      const previousManualChoices = bulkManualChoices instanceof Map ? bulkManualChoices : new Map();
      bulkChosen = new Map();
      ...
      const blocks = segments.map((segment, idx) => {
        ...
        if (uniqueMatches.length > 1) {
          const manualId = previousManualChoices.get(bulkChoiceKey(idx, term));
          const manualStillValid = manualId && uniqueMatches.some(drug => drug.id === manualId) && !selectedDrugs.some(d => d.id === manualId);
          if (manualStillValid) bulkChosen.set(idx, manualId);
    ...
    function bulkLineMulti(term, idx, matches, committed = true) {
      const selectedId = bulkChosen.get(idx);
      const selectedDrug = selectedId ? matches.find(drug => drug.id === selectedId) : null;

      // 複数候補から手動選択済みの行は、再度「選択してください」を出さず、
      // 選択済みチップだけをコンパクトに残す。
      if (selectedDrug) {
        return `
          <div class="bulk-line auto committed manual-choice" data-bulk-line="${idx}">
            <div class="bulk-pill-row">
              <button type="button" class="bulk-pill selected" disabled>${escapeHtml(selectedDrug.brands[0])} <span style="opacity:.6;">← ${escapeHtml(term)}</span></button>
            </div>
          </div>
        `;
      }
        ...
        <div class="bulk-line multi" data-bulk-line="${idx}">
          ...
          <div class="bulk-pill-row">
            ${matches.map(drug => {
              ...
              return `<button type="button" class="bulk-pill" data-bulk-choice="${idx}" data-drug-id="${escapeHtml(drug.id)}"${dup ? " disabled" : ""}>${escapeHtml(drug.brands[0])}${dup ? "（選択済み）" : ""}</button>`;
            }).join("")}
            <button type="button" class="bulk-pill none-choice" data-bulk-choice="${idx}" data-drug-id="">選ばない</button>
          </div>
        </div>
      `;
    }
    ...
    function attachBulkChoiceHandlers(terms) {
      bulkPreview.querySelectorAll("[data-bulk-choice]").forEach(btn => {
        btn.addEventListener("click", () => {
          ...
          const key = bulkChoiceKey(idx, term);
          const drugId = btn.dataset.drugId;
          if (drugId) {
            bulkChosen.set(idx, drugId);
            bulkManualChoices.set(key, drugId);
          } else {
            bulkChosen.delete(idx);
            bulkManualChoices.delete(key);
          }

          // 手動選択後は候補選択行を即座にコンパクトな選択済み表示へ更新し、
          // 「選択してください」の表示を残さない。
          analyzeBulkInput();

          // 候補を手動選択した後も選択状態を保持したまま、
          // すぐ改行して次の薬剤名を入力できるように検索BOXへ戻す。
          setTimeout(() => {
            bulkInput.focus();
            const len = bulkInput.value.length;
            try { bulkInput.setSelectionRange(len, len); } catch (e) {}
          }, 0);
        });
      });
    }
    ...
    function addBulkSelected() {
      ...
      // 結果表示：モーダルを強制的に閉じて結果エリアへ移動
      forceBulkResultTransitionV42();
    }


    function forceBulkResultTransitionV42() {
      ...
      // 2. モーダルを強制的に不可視化。openやariaが残っても勝つforce-closedを使う
      if (bulkModal) {
        bulkModal.classList.add("force-closed");
        bulkModal.classList.remove("open");
        bulkModal.setAttribute("aria-hidden", "true");
      ...
      // 3. スクロールは、キーボードが閉じるタイミングに負けないよう複数回
      const jump = () => {
        if (!target) return;
    ...
    function openDrugDetailModal(drug, mode = currentMode) {
      ...
      drugDetailBody.innerHTML = `
        ...
        <div class="status-grid">
          ${showPregnancy ? statusBox("妊娠の評価", drug.pregnancySummary, drug.pregnancyInsert, statusType(drug.pregnancySummary, drug.pregnancyInsert, "pregnancy"), "pregnancy") : ""}
          ${showLactation ? statusBox("授乳の評価", drug.lactationSummary, drug.lactationInsert, statusType(drug.lactationSummary, drug.lactationInsert, "lactation"), "lactation") : ""}
        </div>
      ...
      bindInsertHelpButtons(drugDetailBody);
    }
    ...
    const ANTIBIOTIC_GROUPS = [
      { key: "penicillin", categoryLabel: "抗菌薬", icon: "💊", label: "ペニシリン系", match: d => d.detailKey === "penicillin" },
      { key: "cephem", categoryLabel: "抗菌薬", icon: "💊", label: "セフェム系", match: d => d.detailKey === "cephem" },
      { key: "macrolide", categoryLabel: "抗菌薬", icon: "💊", label: "マクロライド系", match: d => d.detailKey === "macrolide" },
      { key: "quinolone", categoryLabel: "抗菌薬", icon: "💊", label: "ニューキノロン系", match: d => d.detailKey === "quinolone" },
      { key: "other", categoryLabel: "抗菌薬", icon: "💊", label: "その他（ST合剤・テトラサイクリン等）", match: d => ["carbapenem","penem","monobactam","glycopeptide","fosfomycin","aminoglycoside","tetracycline","lincomycin","oxazolidinone","chloramphenicol","st"].includes(d.detailKey) },
      { key: "antitb", categoryLabel: "抗菌薬", icon: "💊", label: "抗結核薬", match: d => d.detailKey === "antitb" }
    ];

    const ANTIVIRAL_GROUPS = [
      { key: "antiviral_herpes", categoryLabel: "抗ウイルス薬", icon: "🧬", label: "ヘルペス", match: d => d.detailKey === "antiviral_herpes" },
      { key: "antiviral_covid", categoryLabel: "抗ウイルス薬", icon: "🧬", label: "COVID-19", match: d => d.detailKey === "antiviral_covid" },
      { key: "antiviral_flu", categoryLabel: "抗ウイルス薬", icon: "🧬", label: "インフルエンザ", match: d => d.detailKey === "antiviral_flu" }
    ];

    const ANALGESIC_GROUPS = [
      { key: "analgesic_acetaminophen", categoryLabel: "解熱鎮痛薬", icon: "💊", label: "アセトアミノフェン", match: d => d.detailKey === "analgesic_acetaminophen" },
      { key: "analgesic_nsaids", categoryLabel: "解熱鎮痛薬", icon: "💊", label: "NSAIDs", match: d => ["analgesic_basic_nsaid", "analgesic_oral_nsaid", "analgesic_topical_nsaid"].includes(d.detailKey) },
      { key: "analgesic_other_pain", categoryLabel: "解熱鎮痛薬", icon: "💊", label: "その他鎮痛薬", match: d => d.detailKey === "analgesic_other_pain" }
    ];

    const ANTIALLERGY_GROUPS = [
      { key: "antiallergy_h1_1st", categoryLabel: "抗アレルギー薬", icon: "🌼", label: "第1世代抗ヒスタミン薬", match: d => d.detailKey === "antiallergy_h1_1st" },
      { key: "antiallergy_h1_2nd", categoryLabel: "抗アレルギー薬", icon: "🌼", label: "第2世代抗ヒスタミン薬", match: d => d.detailKey === "antiallergy_h1_2nd" },
      { key: "antiallergy_mediator", categoryLabel: "抗アレルギー薬", icon: "🌼", label: "メディエーター遊離抑制薬", match: d => d.detailKey === "antiallergy_mediator" },
      { key: "antiallergy_ltra", categoryLabel: "抗アレルギー薬", icon: "🌼", label: "ロイコトリエン受容体拮抗薬", match: d => d.detailKey === "antiallergy_ltra" },
      { key: "antiallergy_other", categoryLabel: "抗アレルギー薬", icon: "🌼", label: "その他抗アレルギー薬", match: d => d.detailKey === "antiallergy_other" }
    ];

    const DIABETES_GROUPS = [
      { key: "diabetes_insulin", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "インスリン製剤", match: d => d.detailKey === "diabetes_insulin" },
      { key: "diabetes_biguanide", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "ビグアナイド薬", match: d => d.detailKey === "diabetes_biguanide" },
      { key: "diabetes_imeglimin", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "ミトコンドリア機能改善薬", match: d => d.detailKey === "diabetes_imeglimin" },
      { key: "diabetes_thiazolidine", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "チアゾリジン誘導体", match: d => d.detailKey === "diabetes_thiazolidine" },
      { key: "diabetes_su", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "スルホニルウレア薬", match: d => d.detailKey === "diabetes_su" },
      { key: "diabetes_glinide", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "速効型インスリン分泌促進薬", match: d => d.detailKey === "diabetes_glinide" },
      { key: "diabetes_alpha_gi", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "α-グルコシダーゼ阻害薬", match: d => d.detailKey === "diabetes_alpha_gi" },
      { key: "diabetes_sglt2", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "SGLT2阻害薬", match: d => d.detailKey === "diabetes_sglt2" },
      { key: "diabetes_dpp4", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "DPP-4阻害薬", match: d => d.detailKey === "diabetes_dpp4" },
      { key: "diabetes_glp1", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "GLP-1受容体作動薬", match: d => d.detailKey === "diabetes_glp1" },
      { key: "diabetes_gip_glp1", categoryLabel: "糖尿病治療薬", icon: "🍬", label: "GIP/GLP-1受容体作動薬", match: d => d.detailKey === "diabetes_gip_glp1" }
    ];

    const LIPID_GROUPS = [
      { key: "lipid_statin", categoryLabel: "脂質異常症治療薬", icon: "🫒", label: "スタチン系薬剤", match: d => d.detailKey === "lipid_statin" },
      { key: "lipid_ezetimibe", categoryLabel: "脂質異常症治療薬", icon: "🫒", label: "小腸コレステロール阻害薬", match: d => d.detailKey === "lipid_ezetimibe" },
      { key: "lipid_fibrate", categoryLabel: "脂質異常症治療薬", icon: "🫒", label: "フィブラート系薬剤", match: d => d.detailKey === "lipid_fibrate" },
      { key: "lipid_nicotinic", categoryLabel: "脂質異常症治療薬", icon: "🫒", label: "ニコチン酸系薬剤", match: d => d.detailKey === "lipid_nicotinic" },
      { key: "lipid_pufa", categoryLabel: "脂質異常症治療薬", icon: "🫒", label: "多価不飽和脂肪酸", match: d => d.detailKey === "lipid_pufa" }
    ];

    const THYROID_GROUPS = [
      { key: "thyroid_hormone", categoryLabel: "甲状腺疾患治療薬", icon: "🦋", label: "甲状腺ホルモン製剤", match: d => d.detailKey === "thyroid_hormone" },
      { key: "thyroid_antithyroid", categoryLabel: "甲状腺疾患治療薬", icon: "🦋", label: "抗甲状腺薬", match: d => d.detailKey === "thyroid_antithyroid" },
      { key: "thyroid_iodide", categoryLabel: "甲状腺疾患治療薬", icon: "🦋", label: "無機ヨウ素", match: d => d.detailKey === "thyroid_iodide" }
    ];

    const BONE_GROUPS = [
      { key: "bone_vitamin_d", categoryLabel: "骨・カルシウム代謝薬", icon: "🦴", label: "ビタミンD製剤", match: d => d.detailKey === "bone_vitamin_d" },
      { key: "bone_bisphosphonate", categoryLabel: "骨・カルシウム代謝薬", icon: "🦴", label: "ビスホスホネート", match: d => d.detailKey === "bone_bisphosphonate" },
      { key: "bone_vitamin_k", categoryLabel: "骨・カルシウム代謝薬", icon: "🦴", label: "ビタミンK2製剤", match: d => d.detailKey === "bone_vitamin_k" },
      { key: "bone_calcium", categoryLabel: "骨・カルシウム代謝薬", icon: "🦴", label: "カルシウム製剤", match: d => d.detailKey === "bone_calcium" }
    ];

    const CV_GROUPS = [
      { key: "cv_ace", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "ACE阻害薬", match: d => d.detailKey === "cv_ace" },
      { key: "cv_arb", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "ARB", match: d => d.detailKey === "cv_arb" },
      { key: "cv_renin", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "直接的レニン阻害薬", match: d => d.detailKey === "cv_renin" },
      { key: "cv_mra", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "MRA", match: d => d.detailKey === "cv_mra" },
      { key: "cv_ca", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "Ca拮抗薬", match: d => d.detailKey === "cv_ca" },
      { key: "cv_beta", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "β遮断薬", match: d => d.detailKey === "cv_beta" },
      { key: "cv_alpha_beta", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "αβ遮断薬", match: d => d.detailKey === "cv_alpha_beta" },
      { key: "cv_alpha", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "α遮断薬", match: d => d.detailKey === "cv_alpha" },
      { key: "cv_central", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "中枢性交感神経抑制薬", match: d => d.detailKey === "cv_central" },
      { key: "cv_vasodilator", categoryLabel: "循環器｜降圧薬", icon: "🫀", label: "血管拡張薬", match: d => d.detailKey === "cv_vasodilator" },
      { key: "cv_diuretic_loop", categoryLabel: "循環器｜利尿剤", icon: "🫀", label: "ループ利尿薬", match: d => d.detailKey === "cv_diuretic_loop" },
      { key: "cv_diuretic_thiazide", categoryLabel: "循環器｜利尿剤", icon: "🫀", label: "サイアザイド系利尿薬", match: d => d.detailKey === "cv_diuretic_thiazide" }
    ];

    const RESP_GROUPS = [
      { key: "resp_cough", categoryLabel: "呼吸器｜鎮咳去痰", icon: "🫁", label: "鎮咳去痰", match: d => d.detailKey === "resp_cough" }
    ];

    const GI_GROUPS = [
      { key: "gi_upper", categoryLabel: "消化器｜上部消化管", icon: "🫃", label: "上部消化管", match: d => d.detailKey === "gi_upper" },
      { key: "gi_lower", categoryLabel: "消化器｜下部消化管", icon: "🫃", label: "下部消化管", match: d => d.detailKey === "gi_lower" }
    ];

    const COMPARISON_GROUPS = [...ANTIBIOTIC_GROUPS, ...ANTIVIRAL_GROUPS, ...ANALGESIC_GROUPS, ...ANTIALLERGY_GROUPS, ...DIABETES_GROUPS, ...LIPID_GROUPS, ...THYROID_GROUPS, ...BONE_GROUPS, ...CV_GROUPS, ...RESP_GROUPS, ...GI_GROUPS];

    function findComparisonGroupForDrug(drug) {
      return COMPARISON_GROUPS.find(group => group.match(drug));
    }

    function renderSubcategoryButtons(container, groups) {
      if (!container) return;
      ...
      container.querySelectorAll("[data-group]").forEach(btn => {
        btn.addEventListener("click", () => {
          const group = COMPARISON_GROUPS.find(g => g.key === btn.dataset.group);
          if (!group) return;
          openComparison(group);
        });
      });
    ...
    function initCategoryBrowser() {
      if (antibioticCount) antibioticCount.textContent = DRUGS.filter(d => ANTIBIOTIC_GROUPS.some(g => g.match(d))).length;
      if (antiviralCount) antiviralCount.textContent = DRUGS.filter(d => ANTIVIRAL_GROUPS.some(g => g.match(d))).length;
      if (analgesicCount) analgesicCount.textContent = DRUGS.filter(d => ANALGESIC_GROUPS.some(g => g.match(d))).length;
      if (antiallergyCount) antiallergyCount.textContent = DRUGS.filter(d => ANTIALLERGY_GROUPS.some(g => g.match(d))).length;
      if (diabetesCount) diabetesCount.textContent = DRUGS.filter(d => DIABETES_GROUPS.some(g => g.match(d))).length;
      if (lipidCount) lipidCount.textContent = DRUGS.filter(d => LIPID_GROUPS.some(g => g.match(d))).length;
      if (thyroidCount) thyroidCount.textContent = DRUGS.filter(d => THYROID_GROUPS.some(g => g.match(d))).length;
      if (boneCount) boneCount.textContent = DRUGS.filter(d => BONE_GROUPS.some(g => g.match(d))).length;
      if (cvCount) cvCount.textContent = DRUGS.filter(d => CV_GROUPS.some(g => g.match(d))).length;
      if (respCount) respCount.textContent = DRUGS.filter(d => RESP_GROUPS.some(g => g.match(d))).length;
      if (giCount) giCount.textContent = DRUGS.filter(d => GI_GROUPS.some(g => g.match(d))).length;

      renderSubcategoryButtons(antibioticSubButtons, ANTIBIOTIC_GROUPS);
      renderSubcategoryButtons(antiviralSubButtons, ANTIVIRAL_GROUPS);
      renderSubcategoryButtons(analgesicSubButtons, ANALGESIC_GROUPS);
      renderSubcategoryButtons(antiallergySubButtons, ANTIALLERGY_GROUPS);
      renderSubcategoryButtons(diabetesSubButtons, DIABETES_GROUPS);
      renderSubcategoryButtons(lipidSubButtons, LIPID_GROUPS);
      renderSubcategoryButtons(thyroidSubButtons, THYROID_GROUPS);
      renderSubcategoryButtons(boneSubButtons, BONE_GROUPS);
      renderSubcategoryButtons(cvSubButtons, CV_GROUPS);
      renderSubcategoryButtons(respSubButtons, RESP_GROUPS);
      renderSubcategoryButtons(giSubButtons, GI_GROUPS);

      if (antibioticCategoryBtn) {
        antibioticCategoryBtn.addEventListener("click", () => {
          antibioticSubPanel.hidden = !antibioticSubPanel.hidden;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
          if (diabetesSubPanel) diabetesSubPanel.hidden = true;
          if (lipidSubPanel) lipidSubPanel.hidden = true;
          if (thyroidSubPanel) thyroidSubPanel.hidden = true;
          if (boneSubPanel) boneSubPanel.hidden = true;
          if (cvSubPanel) cvSubPanel.hidden = true;
          if (respSubPanel) respSubPanel.hidden = true;
          if (giSubPanel) giSubPanel.hidden = true;
        });
      }

      if (antiviralCategoryBtn) {
        antiviralCategoryBtn.addEventListener("click", () => {
          antiviralSubPanel.hidden = !antiviralSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
          if (diabetesSubPanel) diabetesSubPanel.hidden = true;
          if (lipidSubPanel) lipidSubPanel.hidden = true;
      ...
      if (analgesicCategoryBtn) {
        analgesicCategoryBtn.addEventListener("click", () => {
          analgesicSubPanel.hidden = !analgesicSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
          if (diabetesSubPanel) diabetesSubPanel.hidden = true;
      ...
      if (antiallergyCategoryBtn) {
        antiallergyCategoryBtn.addEventListener("click", () => {
          antiallergySubPanel.hidden = !antiallergySubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (cvSubPanel) cvSubPanel.hidden = true;
      ...
      if (diabetesCategoryBtn) {
        diabetesCategoryBtn.addEventListener("click", () => {
          diabetesSubPanel.hidden = !diabetesSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
      ...
      if (lipidCategoryBtn) {
        lipidCategoryBtn.addEventListener("click", () => {
          lipidSubPanel.hidden = !lipidSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
      ...
      if (thyroidCategoryBtn) {
        thyroidCategoryBtn.addEventListener("click", () => {
          thyroidSubPanel.hidden = !thyroidSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
      ...
      if (boneCategoryBtn) {
        boneCategoryBtn.addEventListener("click", () => {
          boneSubPanel.hidden = !boneSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
      ...
      if (closeAntiviralSubPanelBtn) {
        closeAntiviralSubPanelBtn.addEventListener("click", () => {
          antiviralSubPanel.hidden = true;
        });
      }

      if (closeAnalgesicSubPanelBtn) {
        closeAnalgesicSubPanelBtn.addEventListener("click", () => {
          analgesicSubPanel.hidden = true;
        });
      }

      if (closeAntiallergySubPanelBtn) {
        closeAntiallergySubPanelBtn.addEventListener("click", () => {
          antiallergySubPanel.hidden = true;
        });
      }

      if (closeDiabetesSubPanelBtn) {
        closeDiabetesSubPanelBtn.addEventListener("click", () => {
          diabetesSubPanel.hidden = true;
        });
      }

      if (closeLipidSubPanelBtn) {
        closeLipidSubPanelBtn.addEventListener("click", () => {
          lipidSubPanel.hidden = true;
        });
      }

      if (closeThyroidSubPanelBtn) {
        closeThyroidSubPanelBtn.addEventListener("click", () => {
          thyroidSubPanel.hidden = true;
        });
      }

      if (closeBoneSubPanelBtn) {
        closeBoneSubPanelBtn.addEventListener("click", () => {
          boneSubPanel.hidden = true;
        });
      }

      if (cvCategoryBtn) {
        cvCategoryBtn.addEventListener("click", () => {
          cvSubPanel.hidden = !cvSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
      ...
      if (closeCvSubPanelBtn) {
        closeCvSubPanelBtn.addEventListener("click", () => {
          cvSubPanel.hidden = true;
        });
      }


      if (respCategoryBtn) {
        respCategoryBtn.addEventListener("click", () => {
          respSubPanel.hidden = !respSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
      ...
      if (closeRespSubPanelBtn) {
        closeRespSubPanelBtn.addEventListener("click", () => {
          respSubPanel.hidden = true;
        });
      }

      if (giCategoryBtn) {
        giCategoryBtn.addEventListener("click", () => {
          giSubPanel.hidden = !giSubPanel.hidden;
          if (antibioticSubPanel) antibioticSubPanel.hidden = true;
          if (antiviralSubPanel) antiviralSubPanel.hidden = true;
          if (analgesicSubPanel) analgesicSubPanel.hidden = true;
          if (antiallergySubPanel) antiallergySubPanel.hidden = true;
      ...
      if (closeGiSubPanelBtn) {
        closeGiSubPanelBtn.addEventListener("click", () => {
          giSubPanel.hidden = true;
        });
      }
    ...
    function bindComparisonInteractions() {
      bindInsertHelpButtons(comparisonScroll);
    ...
    function refreshOpenComparison() {
      ...
      compareTitle.textContent = `${currentComparisonGroup.icon || "📋"} ${currentComparisonGroup.categoryLabel || "薬効群"}｜${currentComparisonGroup.label}`;
    ...
    // bulk-result-safety-listener-v42: 既存処理後にも強制的に閉じる保険
    addBulkBtn.addEventListener("click", () => setTimeout(forceBulkResultTransitionV42, 60));
...
// Network-first service worker registration.
    ...
    const APP_VERSION = "20260505-1908";
