const PIN_HASH = "3472adbbcb9677d1b45365d37d96d1c33217d745567577fd9bd5c2766a258320";
    ...
    function renderInsertHelpButtonCompact(rawValue, axis = currentMode) {
      const info = getInsertHelpInfo(rawValue, axis);
      const label = info.label || "記載なし";
      const lines = splitInsertLabelForTable(label);
      const stateClass = getInsertCompactState(label);
      return `
        <button type="button"
    ...
    function splitInsertLabelForTable(label) {
      const value = String(label || "記載なし");
      const fixed = {
        "授乳回避（添文①）": ["授乳回避", "添文①"],
        "授乳しないことが望ましい（添文②）": ["授乳しないことが", "望ましい", "添文②"],
        "継続または中止を検討（添文③）": ["継続または", "中止を検討", "添文③"],
        "投与回避・やむを得ず投与なら授乳中止（添文④）": ["投与回避", "やむを得ず投与なら", "授乳中止", "添文④"],
        "有益性（添文②）": ["有益性", "添文②"],
        "有益性（添文③）": ["有益性", "添文③"],
        "本文参照": ["本文参照"],
        "記載なし": ["記載なし"],
      ...
      if (value.length <= 12) return [value.slice(0, 6), value.slice(6)];
      return [value.slice(0, 6), value.slice(6, 12), value.slice(12, 18)];
    }

    function getInsertCompactState(label) {
      const value = String(label || "");
      if (value.includes("使用不可") || value.includes("禁忌") || value.includes("投与回避") || value.includes("授乳中止")) return "danger";
      if (value.includes("使用可")) return "ok";
      if (value.includes("記載なし") || value.includes("情報なし") || value.includes("情報不足")) return "unknown";
      return "caution";
    }
